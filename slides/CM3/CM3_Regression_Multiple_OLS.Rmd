...
$$
so that the intercept $\mathbb{1} = (1, \dotsc, 1)^T$ is seen as a predictor:
...
Without loss of generality, the previous model can hence be written as:
...
The OLS estimators $\hat{\boldsymbol{\beta}}$ is given by:
...
## Reminder - Euclidean Geometry 2/2 {.build}
...
## Reminder: Matrix of Projection - 2/4
...
## Reminder: Matrix of Projection - 3/4 {.build}

**Property**:  

If $\mathbf{P}$ is an orthogonal projection matrix, then for any 
...
$$
\begin{aligned}
\langle (\mathbf{I}_n - \mathbf{P})\mathbf{x}, \mathbf{P}\mathbf{x} \rangle
&= [\mathbf{P}\mathbf{x}]^T (\mathbf{I}_n - \mathbf{P})\mathbf{x} & \phantom{[idempotent]}
\end{aligned}
$$

$$
\begin{aligned}
\phantom{\langle (\mathbf{I}_n - \mathbf{P})\mathbf{x}, \mathbf{P}\mathbf{x} \rangle }
&= \mathbf{x}^T \mathbf{P}^T (\mathbf{I}_n - \mathbf{P})\mathbf{x}\\
&= \mathbf{x}^T \mathbf{P} (\mathbf{I}_n - \mathbf{P})\mathbf{x} & [symmetric]\\
&= \mathbf{x}^T (\mathbf{P} - \mathbf{P}^2) \mathbf{x}\\
&= 0 & [idempotent]
\end{aligned}
$$
...
An $n$ matrix is said to be *orthogonal* if
...
$\mathbf{\Delta}$ has $p$ "1" and $(1-p)$ "0" on its diagonal,
with $p$ the dimension of $\text{Im}(\mathbf{P})$ (i.e. the rank of $\mathbf{P}$).
...
$\mathcal{M}(\mathbf{X})$.
>* I.e. $\mathbf{y} - \mathbf{X}\hat{\boldsymbol{\beta}}$ is orthogonal to all $\mathbf{x}_i$:
$$
\langle \mathbf{x}_i,  \mathbf{y} - \mathbf{X}\hat{\boldsymbol{\beta}} \rangle
= \mathbf{x}_i^T (\mathbf{y} - \mathbf{X}\hat{\boldsymbol{\beta}}) = 0
\qquad 
1 \leq i \leq n
$$
...
$$
i.e. \quad 
(\mathbf{x}_1 \dotsc \mathbf{x}_n)^T (\mathbf{y} - \mathbf{X}\hat{\boldsymbol{\beta}})
= 
\sum_{i = 1}^n \mathbf{x}_i^T (\mathbf{y} - \mathbf{X}\hat{\boldsymbol{\beta}})
=
0
$$
...
$$
\mathbb{E}[\mathbf{z}] = (\mathbb{E}[z_1], \cdots, \mathbb{E}[z_i])^T
$$
