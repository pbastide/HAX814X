...
<!-- $\mathbb{Var}[\epsilon_i] = \sigma^2$;  -->
<!-- $\mathbb{Cov}[\epsilon_i; \epsilon_j] = 0$; -->
<!-- $1 \leq i \neq j \leq n$) -->
...
# ML
mu <- xobs; sigma2 <- 1
plot(x, dnorm(x, mu, sigma2), type = "l", ylab = "f(x)", col = "firebrick3", lwd = 0)
segments(x0 = mu, y0 = 0, x1 = mu, y1 = dnorm(mu, mu, sigma2), lty = 2, col = "firebrick3", lwd = 2)
axis(side = 1, at = xobs, labels = expression(x[obs]), col = "firebrick3",  col.axis = "firebrick3", lwd = 2)
# mean
mu <- -1; sigma2 <- 1
lines(x, dnorm(x, mu, sigma2), lwd = 2)
segments(x0 = xobs, y0 = dnorm(xobs, mu, sigma2), x1 = -10, y1 = dnorm(xobs, mu, sigma2), lty = 2, lwd = 2)
axis(side = 2, at = dnorm(xobs, mu, sigma2), labels = expression(paste(L(mu == -1*"|"*x[obs]))), las = 1, hadj = 1.2)
```
...
segments(x0 = mu, y0 = 0, x1 = mu, y1 = dnorm(mu, mu, sigma2), lty = 2, col = "firebrick3", lwd = 2)
axis(side = 1, at = xobs, labels = expression(x[obs]), col = "firebrick3",  col.axis = "firebrick3", lwd = 2)
# segments(x0 = xobs, y0 = dnorm(xobs, mu, sigma2), x1 = -10, y1 = dnorm(xobs, mu, sigma2), lty = 2, col = "red")
# axis(side = 2, at = dnorm(xobs, mu, sigma2), labels = expression(paste(L(mu == x[obs]*"|"*x[obs]))), las = 1, col = "red",  col.axis = "red", hadj = 1.2)
# mean
mu <- 0; sigma2 <- 1
lines(x, dnorm(x, mu, sigma2), lwd = 2)
segments(x0 = xobs, y0 = dnorm(xobs, mu, sigma2), x1 = -10, y1 = dnorm(xobs, mu, sigma2), lty = 2, lwd = 2)
axis(side = 2, at = dnorm(xobs, mu, sigma2), labels = expression(paste(L(mu == 0*"|"*x[obs]))), las = 1, hadj = 1.2, lwd = 2)
# mean
mu <- -1; sigma2 <- 1
lines(x, dnorm(x, mu, sigma2), lwd = 0.5)
segments(x0 = xobs, y0 = dnorm(xobs, mu, sigma2), x1 = -10, y1 = dnorm(xobs, mu, sigma2), lty = 2, lwd = 0.5)
axis(side = 2, at = dnorm(xobs, mu, sigma2), labels = expression(paste(L(mu == -1*"|"*x[obs]))), las = 1, hadj = 1.2, lwd = 0.5)
```
...
# ML
mu <- xobs; sigma2 <- 1
plot(x, dnorm(x, mu, sigma2), type = "l", ylab = "f(x)", col = "firebrick3", lwd = 2)
segments(x0 = mu, y0 = 0, x1 = mu, y1 = dnorm(mu, mu, sigma2), lty = 2, col = "firebrick3", lwd = 2)
axis(side = 1, at = xobs, labels = expression(x[obs]), col = "firebrick3",  col.axis = "firebrick3", lwd = 2)
segments(x0 = xobs, y0 = dnorm(xobs, mu, sigma2), x1 = -10, y1 = dnorm(xobs, mu, sigma2), lty = 2, col = "firebrick3", lwd = 2)
axis(side = 2, at = dnorm(xobs, mu, sigma2), labels = expression(paste(L(mu == x[obs]*"|"*x[obs]))), las = 1, col = "firebrick3",  col.axis = "firebrick3", hadj = 1.2)
# mean
mu <- 0; sigma2 <- 1
lines(x, dnorm(x, mu, sigma2), lwd = 0.5)
segments(x0 = xobs, y0 = dnorm(xobs, mu, sigma2), x1 = -10, y1 = dnorm(xobs, mu, sigma2), lty = 2, lwd = 0.5)
axis(side = 2, at = dnorm(xobs, mu, sigma2), labels = expression(paste(L(mu == 0*"|"*x[obs]))), las = 1, hadj = 1.2, lwd = 0.5)
# mean
mu <- -1; sigma2 <- 1
lines(x, dnorm(x, mu, sigma2), lwd = 0.5)
segments(x0 = xobs, y0 = dnorm(xobs, mu, sigma2), x1 = -10, y1 = dnorm(xobs, mu, sigma2), lty = 2, lwd = 0.5)
...
```{r ad1}
ad <- read.csv("../data/Advertising.csv", row.names = "X")
head(ad)
```
...
# Plot
lines(newx, conf_int[, 2], col = "blue", lty = 2)
lines(newx, conf_int[, 3], col = "blue", lty = 2)
# Prediction intervals
pred_int <- predict(fit_tv, data.frame(TV = newx), interval = "pred")
# Plot
lines(newx, pred_int[, 2], col = "red", lty = 2)
lines(newx, pred_int[, 3], col = "red", lty = 2)
```

## Advertising - TV - 5/6

* 95\% confidence interval (blue) and prediction interval (red)
...
<!-- ## Example: Test on $\hat{\beta}_1$ - 1/2 -->

<!-- * **Question**: Does $\mathbf{x}$ have an impact on $\mathbf{y}$ ?   -->

<!-- >* **i.e.** Do we have $\beta_1 = 0$ ?   -->
...
<!-- $$ -->
<!-- T_1 = -->
<!-- \frac{\hat{\beta}_1}{\sqrt{\hat{\sigma}^2(1/s_{\mathbf{x}}^2)/n}} -->
<!-- \underset{\mathcal{H}_0}{\sim} -->
<!-- \mathcal{T}_{n-2} -->
<!-- $$ -->

<!-- ## Example: Test on $\hat{\beta}_1$ - 2/2 -->
...
<!-- ```{r test, echo=FALSE, fig.height=4, fig.width=7} -->
<!-- x <- -5500:5500 / 1000 -->
<!-- ccc <- hcl.colors(6) -->
<!-- n <- 5 -->
<!-- # 1 -->
<!-- p <- n - 2 -->
<!-- plot(x, dt(x, p), type = "l", ylab = "density", ylim = c(0, 0.4), col = ccc[1], lwd = 2, xlab = expression(T[1])) -->
<!-- abline(h = 0) -->
<!-- # quantiles for alp = 5% test -->
<!-- alp <- 0.05 -->
<!-- quants <- qt(c(alp / 2, 1 - alp / 2), p) -->
<!-- segments(x0 = quants, y0 = -1, x1 = quants, y1 = dt(0, p), lty = 2, col = ccc[3]) -->
<!-- axis(side = 1, at = quants, labels = c(expression(q[alpha/2]), expression(q[1 - alpha/2])), col.ticks = ccc[3], col.axis = ccc[3]) -->
<!-- # Color area under the curve -->
<!-- x_low <- -5500:(quants[1]*1000)/1000 -->
<!-- polygon(c(x_low, rev(x_low)), -->
<!--         c(dt(x_low, p), rep(0, length(x_low))), -->
<!--         col = ccc[3], border = NA) -->
<!-- x_high <- (quants[2]*1000):5500/1000 -->
<!-- polygon(c(x_high, rev(x_high)), -->
<!--         c(dt(x_high, p), rep(0, length(x_low))), -->
<!--         col = ccc[3], border = NA) -->
<!-- # legend -->
<!-- legend("topright", -->
<!--        c(expression(T[n-2])), -->
...
* Replace "squared" standard Gaussian by **Fisher** distribution.
