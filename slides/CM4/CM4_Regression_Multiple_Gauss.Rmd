...
&= \frac{1}{\sigma^2}(\mathbf{Y} - \boldsymbol{\mu})^T\mathbf{P}^T \mathbf{P}(\mathbf{Y} - \boldsymbol{\mu}) 
= \cdots \qquad \qquad
\end{aligned}
$$
...
\begin{aligned}
\frac{1}{\sigma^2}\|\mathbf{P}(\mathbf{Y} - \boldsymbol{\mu}) \|^2
&= \frac{1}{\sigma^2}(\mathbf{Y} - \boldsymbol{\mu})^T\mathbf{P}(\mathbf{Y} - \boldsymbol{\mu}) \\
&= \frac{1}{\sigma^2}(\mathbf{Y} - \boldsymbol{\mu})^T\mathbf{U}\mathbf{\Delta}\mathbf{U}^T(\mathbf{Y} - \boldsymbol{\mu})
\quad [\mathbf{\Delta}^2 = \mathbf{\Delta}]\\
&= \frac{1}{\sigma^2}(\mathbf{Y} - \boldsymbol{\mu})^T(\mathbf{\Delta}\mathbf{U}^T)^T\mathbf{\Delta}\mathbf{U}^T(\mathbf{Y} - \boldsymbol{\mu})\\
...
\text{and}
\quad
\frac{n-2}{\sigma^2}\hat{\sigma}^2 \sim \chi^2_{n-p}
$$
...
\qquad\qquad\qquad \iff \mathcal{R}_\alpha = \left\{ t \in \mathbb{R}  ~|~ |T_k| \geq t_{n-p}(1 - \alpha/2) \right\}
$$
