...
Assumes that $\boldsymbol{\mu}$ can be approximated as $\mathbf{X} \boldsymbol{\beta}$,
...
Assumes that $\boldsymbol{\mu}$ can be approximated as $\mathbf{X}_{\eta} \boldsymbol{\beta}_{\eta}$,
...
>* If $\hat{\sigma}^2$ is unbiaised, then:
$$
...
$\to$ Minimizing Mallow's $C_p$ is the same as minimizing an unbiaised estimate of the risk.
...
$$
\begin{aligned}
(\hat{\boldsymbol{\beta}}, \hat{\sigma}^2_{ML}) 
...
And: 
$$
\hat{\sigma}^2_{ML} = \frac{ \|\mathbf{y} - \hat{\mathbf{y}}_\eta \|^2} {n}
$$
...
>* Model selecition is still an active area of research.
