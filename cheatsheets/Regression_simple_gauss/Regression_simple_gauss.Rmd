...
On a aussi l'estimateur du maximum de vraisemblance pour $\sigma^2$ :$\hat{\sigma}^2_{ML} = \frac{1}{n}\sum_{i = 1}^n \hat{\epsilon}_i^2$ Cependant, celui ci est baisé : $\mathbb{E}[\hat{\sigma}^2_{ML}] = \frac{n - 2}{n}\sigma^2 \neq \sigma^2$.
...
On a donc $\frac{(n-2)\hat\sigma^2}{\sigma^2} \sim \chi^2_{n-2}$ c'est à dire que $\hat\sigma^2$, une fois renormalisée, suit une loi du $\chi^2$ à $n-2$ degrés de liberté.
...
$$ Etant donné que $\mathbf{V}_n^{-1}$ est définie positive, c'est l'équation d'une région de confiance élliptique.
...
La distribution jointe nous donne une éllipse de confiance pour $\hat\beta$.

La région de confiance correspond donc à l'éllipse de confiance.
