...
Il y a $n-p$ degrés de liberté, et si $X^1=1$ est l'intercept alors il y
...
### [b) Biais et variance de $\hat{\boldsymbol{\epsilon}}$ :]{.ul}
...
\hat{\boldsymbol{\epsilon}} = \mathbf{P}^{\mathbf{X}^\bot}\boldsymbol{\epsilon}
$$
...
### [c) Biais et variance de $\hat{\boldsymbol{y}}$ :]{.ul}

$$
\hat{\mathbf{y}}=\mathbf{X}\hat{\boldsymbol{\beta}}=\mathbf{P}^{\mathbf{X}}\mathbf{y}
$$
...
$$
\hat{\sigma}^2 = \frac{1}{n - p} \| \hat{\epsilon} \|^2 =\frac{1}{n - p} \|y-\hat{y}\|^2 = \frac{1}{n - p} RSS
$$

c'est un estimateur sans biais car :

$$
\mathbb{E}[\| \hat{\epsilon} \|^2]= \sigma^2  (n - p)
$$
...
$$
\text{ On a   :}~~~~~~ 
\hat{\sigma}^2 =  \frac{\|\mathbf{P}^{\mathbf{X}^\bot}\mathbf{y} \|^2}{n-p}~~~~~~  \text{ et  :}~~~~~~  \hat{\boldsymbol{\beta}} =  (\mathbf{X}^{T}\mathbf{X})^{-1}\mathbf{X}^{T}\mathbf{P}^{\mathbf{X}}\mathbf{y}\\
\text{ D'aprés le théoreme de cochran :} \\
\mathbf{P}\mathbf{Y}  ~~~\text{et} ~~~\mathbf{P}^{\bot}\mathbf{Y}~~~ \text{ sont indépendants et donc} ~~~~ \hat{\boldsymbol{\beta}} \text{ et } \hat{\sigma}^2 \text{ sont indépendants}
$$

### [e) Erreur de prédiction $\hat{\boldsymbol{\epsilon}}_{n+1}$ :]{.ul}
...
### [d) Distribution de $\hat{\boldsymbol{y}}$ :]{.ul}
...
### [e) Distribution de $\hat{\boldsymbol{\epsilon}}_{n+1}$ (l'erreur de prédiction) :]{.ul}

-   Quand $\sigma^2$ est connu : $$
    \hat{\boldsymbol{\epsilon}}_{n+1}
    \sim
    \mathcal{N}\left(
    0~~;~ 
    ...
    $$
-   Quand $\sigma^2$ est inconnu : $$
    \hat{\boldsymbol{\epsilon}}_{n+1}=y_{n+1}-\hat{y}_{n+1}
    \sim
    \mathcal{N}\left(
    0~~;~ 
    \hat{\sigma}^2 \left(1 + \mathbf{x}^{n+1} (\mathbf{X}^T\mathbf{X})^{-1}(\mathbf{x}^{n+1})^T\right)
    \right)
    $$
...
On rejette (risque d'erreur 5%) $H_0$ pour les deux coefficients
...
-   Si $\|\hat{\mathbf{\epsilon}}\|^2$ est **petit** alors l'erreur est
...
-   Si $\|\hat{\mathbf{\epsilon}}\|^2$ est **grand** alors l'erreur est
...
rejeter $H_0$ car la $pvalue<0.05$ et donc le mdèle est utile. Autrement
...
coefficients sont nuls et donc le modéle est inutile.
...
$$
Avec $\mathbf{X} = (\mathbf{X}_0 ~ \mathbf{X}_1)$,
$rg(\mathbf{X}) = p$ et
$rg(\mathbf{X}_0) = p_0$.
...
- Si $\|y-\hat{y_0}\|^2$ est grande par rapport à $\|y-\hat{y}\|^2$  alors F tend a être grande et donc on tend a rejeter $H_0$ ce qui signifie que le modèle complet est utile.
...
- Si $\|y-\hat{y_0}\|^2$ est petite par rapport à $\|y-\hat{y}\|^2$  alors F tend a être petite et donc on tend à ne pas rejeter  $H_0$ ce que le modèle complet n'apporte pas beaucoup d'informations et donc le petit modèle suffit.

**Le petit modéle est suffisant ** 
