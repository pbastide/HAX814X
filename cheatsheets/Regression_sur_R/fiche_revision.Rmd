...
- $\epsilon_i$ est le terme d’erreur, assumé être distribué selon une loi Normale $\epsilon_i \sim \mathcal{N}(0,\sigma^2)$
...
à savoir préciser la loi des erreurs. Nous supposerons ici que les erreurs sont gaussiennes. Les
hypothèses sont alors : 
...
```{r}
predict(pricereg2, newdata=data.frame(area=800,bedrooms=2), se.fit=TRUE, interval = "prediction", level = 0.95)
```
...
En posant $(H_0)$ : $\beta_1 = \beta_2 = ... =\beta_p = 0$ vs $(H_1)$ : $\exists i \in [\![1;n]\!]$ tel que $\beta_i \neq 0$ 
